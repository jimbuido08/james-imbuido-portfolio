...
Phase 1 (§28) complete: the shared visual foundation is in place — dark-only design tokens (colors, fonts, focus ring, reduced-motion handling) in `app/globals.css`, the `cx` classname helper in `lib/utils.ts`, and UI primitives in `components/ui/` (`Container`, `Button`, `Card` + sub-components, `SectionHeading`, `Tag`). The site shell is built: sticky `Header` (Client Component, `usePathname` active state, accessible mobile disclosure) and `Footer` in `components/navigation/`, wired into `app/layout.tsx`. The homepage and an unlinked `/design` token/primitives preview page are built on these primitives. No feature dependencies (three.js/R3F, Framer Motion, Supabase clients, chess.js) and no portfolio routes exist yet; they land in later phases per the §34 build order. Next up: **Phase 2 — Data Universe prototype** per §34. The §15 directory skeleton exists as empty folders tracked via `.gitkeep`.
