...
Seven placeholder routes (`/about`, `/experience`, `/ai-ml`, `/data`, `/jtb`, `/chess`, `/contact`) now render honest, clearly-marked placeholder copy with per-route metadata — they previously 404'd. No real project content is connected. **Phase 4 complete** — `/chess` is fully playable: a token-built board (tap-tap, drag, full keyboard support), chess.js-enforced legality, three difficulty levels, all game-end states, and an honest client-side heuristic stand-in opponent behind the `ChessOpponent` interface the trained model will later implement. The isomorphic engine (`lib/chess/`, incl. the `replayMoves` Phase-7 seam) is shared by UI and future server-side win verification. Next up: **Phase 5 — Supabase auth** per §34.
