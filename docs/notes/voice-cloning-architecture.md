...
6. **`voice-voc-chunk.onnx`** — the **shippable vocoder form** (added during the
   ...
   u [200, 512]` → `samples [1, 200] float32, next_h1, next_h2, next_x_prev`).
   Step i consumes `mels[i : i+1]` and `aux[i, i*32:(i+1)*32]` — exactly
   `generate()`'s per-sample rows. JS seeds `u ~ Uniform[0, 1)` **per
   (sample, class)** (u = 0.5 everywhere ⇒ argmax, used by fixtures);
   gumbel-max `argmax(logits + −log(−log u))` samples exactly from the
   categorical. The GRU hidden states are loop-carried in-graph and returned
   as `next_h1`/`next_h2` (`generate()` updates them every sample — see the
   postmortem below). Mu-law decode + de-emphasis stay in TS (vectorised
   after all frames).
   ...
   mel frame amortises that 200× → 320 runs per 4 s, ~10–17 s.
...
**Three bugs the fixtures did NOT catch (fixed 2026-09-07, found while
preparing the listening test — the shipped voc-chunk produced a DC ramp, not
speech).** The fixture gate passed 52/52 the whole time because its golden
values were generated by the *same* broken chunk graph — vacuous parity: both
sides computed the same wrong function, and all four fixture `voc_frames`
samples were 0. The DC ramp's signature: constant class-0 samples → mu-law
decode of class 0 = −1.0 → de-emphasis integrates to DC gain
1/(1 − 0.97) = 33.33 (peak/RMS ≈ 33.33, voiced fraction 1.0). Export-side,
`synthesize_sample.py` now runs a speech-sanity diagnostic (peak/RMS/voiced
fraction/envelope-vs-mel correlation) on every generated sample so a
degenerate output can never again be called success.

1. **Frozen GRU state.** The traced chunk graph never updated h1/h2 between
   the 200 unrolled steps — every sample ran off zero state, i.e. the vocoder
   had no memory and no signal continuity. `VocStep.forward` now returns
   `(logits, h1, h2)` with `h1 = rnn1(x, h1); x = h1 + res; h2 = rnn2(cat(x, a2), h2); x = h2 + res`,
   and `VocChunk` threads them across the unroll and exports them as
   `next_h1`/`next_h2` (mirroring `generate()`, which updates both cells every
   sample).
2. **NaN gumbel via Python precedence.** `−torch.log(u).clamp_min(1e-10)`
   clamps *before* the unary minus binds (method call on the log result), so
   u = 0.5 produced `log(−1e-10) = NaN` → argmax over NaN logits always
   returns class 0. Fix: clamp the *inner* positive quantity only —
   `inner = (−log(u)).clamp_min(1e-10); gumbel = −log(inner)` — and never
   clamp the outer value (the gumbel is legitimately negative for u > 1/e).
3. **Scalar noise can't sample.** Even with (1) and (2) fixed, `u` was one
   scalar per sample: adding a constant to all 512 logits shifts them
   uniformly, and `argmax(logits + const) = argmax(logits)` — pure argmax
   masquerading as gumbel-max sampling. The gumbel-max trick requires
   **independent noise per class**: `u` is now `[samples, 512]`
   (`VOC_CHUNK_U_DRAWS = 200 × 512` draws per frame in
   `lib/voice/modelContract.ts`; `engine.ts` fills one uniform per entry).
   The chunk export asserts u-sensitivity: re-running with
   `torch.rand(200, 512)` must change the output vs u = 0.5, or export aborts.

Verification after the fix: export parity < 2e-3, 52/52 wasm gate with
non-degenerate fixture voc frames, reference-pipeline ground truth
(`training/voice/ground_truth.py` — reference Tacotron mel vs ONNX mel
Pearson 0.857; reference `model.generate` on the ONNX mel produces good
audio), and the headless-Chrome drive output re-measured: 4.24 s WAV,
peak 0.531, RMS 0.052, voiced 0.75 — a natural speech envelope, not a DC
ramp.
...
| voc chunk (shipping form) | per-frame ms × 320 frames (4 s audio) | < 20 s @ 4 s | 53.2 ms → 17.0 s | ~53 ms → ~17 s |
| end-to-end | synth (10 words) + vocoder (4 s) | < 45 s | ~12–14 s | 24.3 s (first drive incl. ~110 MB model fetch; warm cache lower) |

(Voc-chunk timing after the per-class gumbel fix — §2.6 — which added ~8 % over
the pre-fix 49.5 ms/frame; the pre-fix numbers were produced by a graph that
shipped a DC ramp, so the post-fix figures are the honest ones.)
...
1. **WaveRNN chunked — now the PRIMARY path, gate-passed.** §1's FLOP budget
   ...
   mel-frame chunk graph (§2.6) lands at ~17 s per 4 s of audio — inside the
   20 s gate. Listening-proxy diagnostics on real-speech inputs put the
   shipping random-u path ahead of the Griffin-Lim floor (envelope correlation
   with the requested mel 0.62–0.70 vs 0.47); the human listening test and the
   Safari column remain James's rows (§5).
...
3. **Compression: both ladders fail — fp32 ships (Milestone B finding).**
   ...
   voc-chunk 17.4 MB ≈ 111 MB total, lazy-loaded per stage; voc-step not
...
2. **WaveRNN viability — machine-verified, human judgment pending**: speed
   ...
   clones (WaveRNN random-u vs argmax vs Griffin-Lim) from real reference-repo
   speakers with speech-sanity diagnostics — the shipping random-u path shows
   a natural envelope (voiced 0.75, envelope-vs-mel r 0.62–0.70 vs the
   Griffin-Lim floor's 0.47) and the browser drive output is speech-shaped.
   What no script can decide is whether it *sounds* right (chunked in-graph
   gumbel + stripped PreNet dropout) — the actual listening test (your own mic
   at /voice, A/B against `training/voice/samples/`) is still yours. The
